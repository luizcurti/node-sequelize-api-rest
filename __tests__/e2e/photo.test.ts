...
import { createUserAndLogin, createStudent, CreatedStudent } from './helpers';
...
describe('E2E - Photo Routes (/photos)', () => {
  ...
  describe('POST /photos/', () => {
    ...
    it('should return 401 without token', async () => {
      // loginRequired runs before multer, so the auth gate is proven without streaming a
      // file - attaching one here races the server's early response against the upload.
      const res = await request(app)
        .post('/photos/')
        .field('student_id', String(student.id));
