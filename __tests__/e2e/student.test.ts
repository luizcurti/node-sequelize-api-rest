  ...
  describe('POST /students/', () => {
    ...
    it('should return 401 without token', async () => {
      const res = await request(app)
        .post('/students/')
        .send({
          name: 'No Token', lastname: 'Test', email: uniqueEmail(), age: 20, weight: 60, height: 160,
        });
    ...
    it('should return 400 with invalid email', async () => {
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${token}`)
        .send({
          name: 'Email Inv', lastname: 'Alido', email: 'invalid', age: 20, weight: 60, height: 160,
        });
    ...
    it('should return 400 with name too short', async () => {
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${token}`)
        .send({
          name: 'AB', lastname: 'Valido', email: uniqueEmail(), age: 20, weight: 60, height: 160,
        });
    ...
    it('should return 400 with duplicate email', async () => {
      ...
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${token}`)
        .send({
          name: 'Dup Student', lastname: 'Last', email, age: 20, weight: 60, height: 160,
        });
